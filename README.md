...
```json
{ "plugin": "build", "requires": { "dimension": ">=0.10.4" } }
```

The generator lifts it into the pack's catalog entry, so the Store can say
"Requires Dimension ≥ 0.10.4 — update Dimension" on a card **before** anything is
...
floor; `packs/build` is the worked example (it pins the bundled `aether-chat`
companion, which first ships in Dimension 0.10.4).
